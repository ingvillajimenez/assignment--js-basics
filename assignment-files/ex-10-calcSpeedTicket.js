...
function calcSpeedTicket(speedLimit, userSpeed, protectedArea){
  var speedingTicket = userSpeed - speedLimit;
  var fine

  if(userSpeed <= speedLimit){
    fine = 0;
  }
  else if(userSpeed > speedLimit && speedingTicket < 20 && protectedArea === false){
    fine = 150;
  }
  else if(userSpeed > speedLimit && speedingTicket >= 20 && protectedArea === false){
    fine = 250;
  }
  else if(userSpeed > speedLimit && speedingTicket < 20 && protectedArea === true){
    fine = 300;
  }
  else if(userSpeed > speedLimit && speedingTicket >= 20 && protectedArea === true){
    fine = 500;
